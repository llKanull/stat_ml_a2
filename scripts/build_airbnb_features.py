...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--snapshot-split",
        choices=("all", "latest", "before-latest", "previous"),
        default="all",
        help=(
            "Use all snapshots, only each city's latest snapshot, all snapshots before latest, "
            "or the previous snapshot before latest."
        ),
    )
...
def main() -> None:
    ...
    config = BuildConfig(
        ...
        snapshot_split=args.snapshot_split,
    )
    if args.chunked:
        tasks = [
