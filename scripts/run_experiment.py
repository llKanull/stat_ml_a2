...
from comp90051_project.tuning import tune_hyperparameters
...
def _run_combo(
    *,
    ...
    for fold_i, (outer_train_idx, outer_test_idx) in enumerate(outer_splits):
        ...
        inner_splits = list(
            k_fold_indices(len(X_tr), k=n_inner, random_state=fold_i * 17)
        )
...
def _summarise(fold_records: list[dict]) -> pd.DataFrame:
    ...
    for (model, features), group in df.groupby(["model", "features"]):
        row: dict = {"model": model, "features": features}
...
def main() -> None:
    ...
    parser.add_argument("--outer-k", type=int, default=10,
                        help="Outer CV folds (spec requires >= 10).")
    parser.add_argument("--inner-k", type=int, default=3,
                        help="Inner CV folds for tuning (spec requires >= 3).")
    ...
    # Outer CV splits 
    outer_splits = list(
        k_fold_indices(len(df), k=args.outer_k, random_state=args.random_state)
    )
    ...
    print(f"{'═' * 72}")
    header = f"{'Model':14s}  {'Features':15s}  {'Accuracy':>15s}  {'Macro-F1':>15s}  {'ROC-AUC':>15s}"
    print(header)
    print("─" * len(header))
