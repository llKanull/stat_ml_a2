...
def main():
    ...
    print(f"  Train: {len(df_train):,} rows  pos={df_train['target_high_demand'].mean():.3f}"
          f"  threshold={threshold:.4f}")
    print(f"  Test:  {len(df_test):,} rows   pos={df_test['target_high_demand'].mean():.3f}"
          f"  cities={sorted(df_test['city'].unique())}")
    print(f"  Gen:   {len(df_gen):,} rows   pos={df_gen['target_high_demand'].mean():.3f}"
    ...
    for feat_name, feat_cols in feat_sets.items():
        ...
        for model_name in model_names:
            ...
            # Temporal test
            rec_test = evaluate(
                model, X_te_imp, y_test,
                f"Temporal test  n={len(y_test):,}  (training cities, Jan 2026)"
            )
            rec_test.update({
                "model": model_name, "features": feat_name, "split": "temporal_test"
            })
            all_records.append(rec_test)
 
            # Geographic generalisation 
            rec_gen = evaluate(
                model, X_ge_imp, y_gen,
                f"Geographic gen n={len(y_gen):,}  (Sydney / Tasmania / WA)"
            )
            rec_gen.update({
                "model": model_name, "features": feat_name, "split": "geographic_gen"
            })
    ...
    for r in all_records:
        print(f"  {r['model']:14s} {r['features']:15s} {r['split']:16s}"
